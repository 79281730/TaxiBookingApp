...
int main() {
  cout << endl << "Welcome to TaxiBookingApp" << endl;
  int opt1, opt2;

start:
  opt1 = menu1();
  if (opt1 == 3) {
    cout << endl << "Thank you for using TaxiBookingApp!" << endl;
    return 0;
  }
  ...
  if (opt1 == 1) {
    ...
    if (opt2 == 2) { // driver
      ...
      askVehicle(false, type, yearOfManufacture, make, model, trimLevel,
                 plateNum, color);

      Vehicle vehicle(type, yearOfManufacture, make, model, trimLevel, plateNum,
                      color);

      int newId = getLastId("drivers.csv") + 1;
      Driver driver(newId, day, month, year, firstName, lastName, phoneNum,
                    password, nic, vehicle);
      driver.appendToFile();
    } else { // passenger
      int newId = getLastId("passengers.csv") + 1;
      Passenger passenger(newId, day, month, year, firstName, lastName,
                          phoneNum, password);
    ...
    // it is already stored in opt2 whether the user wants to be logged in as
    // driver or passenger
    ...
    if (opt2 == 1) {
      ...
      if (passengers_in) {
        while (!found && getline(passengers_in, line)) {
          ...
          */
          ...
          string *fields[8] = {&idStr,       &dayStr,       &monthStr,
                               &yearStr,     &firstNameStr, &lastNameStr,
                               &phoneNumStr, &passwordStr};
      ...
      if (!found) {
        ...
        Passenger passenger(stoi(idStr), stoi(dayStr), stoi(monthStr),
                            stoi(yearStr), firstNameStr, lastNameStr,
                            phoneNumStr, passwordStr);
      ...
      if (!found) {
        ...
        Vehicle vehicle(typeStr, stoi(yearOfManufactureStr), modelStr, makeStr,
                        trimLevelStr, plateNumStr, colorStr);
        Driver driver(stoi(idStr), stoi(dayStr), stoi(monthStr), stoi(yearStr),
                      firstNameStr, lastNameStr, phoneNumStr, passwordStr,
                      nicStr, vehicle);
      ...
      driverMenu:
        ...
        if (opt == 1) {
          ...
          if (found) {
            ...
            updateBooking(booking, "started", driver.getId(), startedAt,
            ...
            updateBooking(booking, "completed", driver.getId(), startedAt,
                          completedAt);
            cout << endl << "Ride completed!" << endl;
            cout << endl
                 << "You may see your updated rating after logging in again"
                 << endl;
          }
        }
        if (opt == 2)

...
inline void askName(string &name, bool first, bool editing) {
  bool is_err;
  do {
    cout << endl << "Enter " << (first ? "first" : "last") << " name: ";
    cin >> name;
    ...
    if (is_err)
      cerr << endl
           << "Error: each of the first and last names must contain at least 1 "
              "and at most 30 alphanumeric characters"
           << endl;
  } while (is_err);
}

inline void askDate(int type, int &date, int min, int max, bool editing) {
  string str;
...
inline void askPhoneNum(string &phoneNum, bool isPassenger, bool editing) {
...
inline void askNIC(bool editing, string &nic) {
  bool nicExists;
  ...
  do {
    ...
    if (nicExists)
      cerr << endl << "Error: this NIC is already in use" << endl;
  } while (is_err || nicExists);
}

inline void askPassword(string &password, string currentPassword,
                        bool editing) {
  ...
  if (editing) // ask for current password
  {
    ...
    do {
      ...
      if (is_err)
        cerr << endl << "Error: invalid password" << endl;
    } while (is_err);
  }
  ...
  if (!doNotModify) {
    ...
    do {
      ...
      cout << endl << "Enter " << newWord << "password: ";
      cin >> password;

      cout << endl << "Confirm " << newWord << "password: ";
      cin >> confirmPassword;
...
inline void askVehicle(bool editing, string &type, int &yearOfManufacture,
                       string &make, string &model, string &trimLevel,
                       string &plateNum, string &color) {
  int minYearOfManufacture = current_year - 17;

  cout << endl << "Enter a number to select the type of your vehicle: " << endl;
  type = typesMenu(editing);

  do {
    cout << endl
         << "Enter year of manufacture of your vehicle ["
         << minYearOfManufacture << " - " << current_year << "]: ";
    cin >> yearOfManufacture;

    if (editing && yearOfManufacture == -1)
      break;
  } while (yearOfManufacture < minYearOfManufacture ||
           yearOfManufacture > current_year);

  do {
    cout << endl << "Enter make (manufacturer) of the vehicle [e.g. Toyota]: ";
    cin >> make;
  ...
  do {
    cout << endl << "Enter model of the vehicle [e.g. Corolla]: ";
    cin >> model;
  ...
  do {
    ...
    cout << endl << "Enter trim level of the vehicle [e.g. LE]: ";
    cin >> trimLevel;
  ...
  do {
    ...
    cout << endl << "Enter plate number of the vehicle [e.g. AAA-1111]: ";
    cin >> plateNum;
  ...
  do {
    cout << endl << "Enter color of the vehicle [e.g. blue]: ";
    cin >> color;
